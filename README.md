...
[Домашня работа №10](#HW10)
[Домашня работа №11](#HW11)
...
## Примечание к ДЗ№10
- Выполнил задания * и **
- Сделал вложеные roles (чутка покосячил с handlers)
- научился создавать test таски в travis
<<<<<<< HEAD
- Допилил скрипт для разных окружений (добавляем переменый в окружени)
- Сделал вложенные роли (app,mongodb)
- Научился пользоваться ansible-galaxy
- Научлс пользоватьс Волт (для шифрования)
- Допилил скрипт для разных окружений (добавляем переменый в окружение)
- Сделал вложенные роли (app,mongodb)
- Научился пользоваться ansible-galaxy
- Научлс пользоватьс Волт (для шифрования)
...
### Работу произвошу на компе с OS Windows10Pro(являюсь сис. админом, установил Windows, чтоб быть ближе к пользователям по окружению)
...
- Пришлось развернуть linux и на нем отестить сборку ansible-Vagrant (на Windows так и не взлетело, жалко)
- Тестирование ролей через молекулу решил проводить через docker(узнал много нового)
